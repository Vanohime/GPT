...
class CasualSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #attention (matrix mask (T, T))
        #att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        #att = att.masked_fill(self.bias[:, :, :T, :T] == 0, float('-inf'))
        #att = F.softmax(att, dim=-1) #makes rows sum to 1
        #y = att @ v
...
class GPT(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        #we want linear layer before classifier and token embeddings be the same tensor, as in original paper
        self.transformer.wte.weight = self.lm_head.weight
        #initialize params as in original code
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb #embeddings

        for block in self.transformer.h: #go through all the blocks
            x = block(x)
    ...
    def configure_optimizer(self, weight_decay, learning_rate, device):
        ...
        #create optim groups
        #we dont want to decay 1-dimensional tensors like biases etc
    ...
    @classmethod
    def from_pretrained(cls, model_type, transformers=None):
...
while x.size(1) < max_length:
    with torch.no_grad():
        logits = model(x)
        #get probs prom last logits
        probs = F.softmax(logits[:, -1, :], dim=-1)
        #get top 50 most likely tokens (k = 50)
